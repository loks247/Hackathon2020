...
@Autowired
	...
	private Map<Integer, Integer> generateConnections(List<Employee> empList) {
		...
		if (empList.size() == 1) {
			fromToMap.put(empList.get(0).getEmpId(), empList.get(0).getEmpId());
			return fromToMap;
		} else if (empList.size() == 2) {
			fromToMap.put(empList.get(0).getEmpId(), empList.get(1).getEmpId());
			fromToMap.put(empList.get(1).getEmpId(), empList.get(0).getEmpId());
			return fromToMap;
		} else {
